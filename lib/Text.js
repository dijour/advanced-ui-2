...
class Text extends GraphicalObject {
    ...
    draw(ctx) {
        ...
        return new Promise((resolve, reject) => {
            resolve(ctx.fillText(this.text, this.x, this.y));
        })
    }
   ...
   /* moves the object to the specified coordinates */
    moveTo(x, y) {
        ...
        // let newX = Math.abs((x1 - x));
        // console.log(newX);
        // let newY = Math.abs((y1 - y));
        // console.log(newY)
        this.x = x + parseInt(coordinates.x);
        this.y = y + parseInt(coordinates.y);
        console.log(this.x);
        console.log(this.y)
    } 
